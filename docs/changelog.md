...
- **New unit tests:** `backend/tests/test_installer_ssl.py` (18 cases). Covers IP vs DNS SAN branching, Caddyfile generation for all four modes including staging, and `_validate_cert_pair` against match / mismatch / expired / near-expiry / domain-mismatch / valid-chain / bad-chain fixtures. All green locally.
- **New CLI flag:** `--le-staging` (requires `--domain`).
- **New `.env` key:** `SSL_LE_STAGING` (blank or `true`).
