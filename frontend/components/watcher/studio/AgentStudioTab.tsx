...
export default function AgentStudioTab() {
  ...
  const handleGhostDoubleClick = useCallback((agentId: number) => {
    const ghostNode = builder.ghostNodes.find(n => (n.data as any).agentId === agentId)
    if (ghostNode) {
      setConfigPanel({ nodeId: ghostNode.id, nodeType: 'builder-ghost-agent', nodeData: ghostNode.data })
    }
  }, [builder.ghostNodes])
    ...
    <div className="space-y-4 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {selectedAgentId && builder.state.attachedSkills.some(s => s.skillType === 'agent_communication') && (
            <button
              onClick={() => builder.setShowA2ANetwork(!builder.showA2ANetwork)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all flex items-center gap-1.5 ${builder.showA2ANetwork ? 'bg-amber-500/20 text-amber-300 border-amber-500/40' : 'bg-tsushin-surface text-tsushin-slate border-tsushin-border hover:border-tsushin-muted'}`}
              title="Show peer agents with A2A permissions as ghost nodes"
            >
        ...
        {selectedAgentId ? (
          <>
            ...
            <StudioCanvasComponent
              ...
              onNodeDoubleClick={handleNodeDoubleClick} onReady={handleCanvasReady}
              onExpandAll={builder.expandAll} onCollapseAll={builder.collapseAll}
              onResetLayout={builder.resetLayout} hasAnyExpanded={builder.expandedCategories.size > 0}
              ghostNodes={builder.ghostNodes} a2aEdges={builder.a2aStudioEdges}
              onGhostDoubleClick={handleGhostDoubleClick}
            />
            {configPanel && (() => {
              const liveNode = builder.nodes.find(n => n.id === configPanel.nodeId)
                ?? builder.ghostNodes.find(n => n.id === configPanel.nodeId)
