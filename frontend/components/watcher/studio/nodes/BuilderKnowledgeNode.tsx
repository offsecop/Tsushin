...
function BuilderKnowledgeNode({ data, selected }: NodeProps) {
  ...
  return (
    <div role="group" aria-label={`Knowledge: ${d.filename}`}
      className={`group builder-node builder-node-knowledge px-4 py-3 rounded-xl border transition-all duration-200 ${selected ? 'border-violet-400 shadow-glow-sm' : 'border-tsushin-border hover:border-violet-400/50'} bg-tsushin-surface`}>
      <Handle type="target" position={Position.Top} className="!bg-violet-400 !border-tsushin-surface !w-3 !h-3" />
      {d.onDetach && <NodeRemoveButton onDetach={d.onDetach} label={`Remove document ${d.filename}`} />}
