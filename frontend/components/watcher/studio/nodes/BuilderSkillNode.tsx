...
function BuilderSkillNode({ data, selected }: NodeProps) {
  ...
  return (
    <div role="group" aria-label={`Skill: ${d.skillName}`}
      className={`group builder-node builder-node-skill px-4 py-3 rounded-xl border transition-all duration-200 ${selected ? 'border-teal-400 shadow-glow-sm' : 'border-tsushin-border hover:border-teal-400/50'} bg-tsushin-surface`}>
      <Handle type="target" position={Position.Top} className="!bg-teal-400 !border-tsushin-surface !w-3 !h-3" />
      {d.onDetach && <NodeRemoveButton onDetach={d.onDetach} label={`Remove skill ${d.skillName}`} />}
