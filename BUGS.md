...
**Scope:** Audit-only disposable fresh install on macOS from a gitignored clone at `.private/installations/fresh-install-v060-20260411-081937/tsushin`, with the original local `tsushin` stack stopped and later restored from a captured manifest. Coverage combined API probes, Playwright browser automation, provider and vector-store setup, Quick Tunnel public-access validation, Playground/knowledge/flows/A2A/custom-skills/MCP/tooling paths, and WhatsApp QR-surface validation. Final WhatsApp message exchange was skipped per user direction after QR blockers were documented.

**Total findings: 8**
- **New:** `BUG-519`, `BUG-520`, `BUG-521`, `BUG-522`, `BUG-523`, `BUG-524`
- **Revalidated open:** `BUG-515`, `BUG-516`
- **Validated working paths:** installer + setup wizard, Qdrant auto-provisioning plus manual vector-store provisioning, provider-instance creation, Ollama discovery, API v1 OAuth2 and generated client, Playground text/image chat, slash commands including `/inject`, Sentinel/MemGuard, MCP server creation, knowledge upload and retrieval, flows (programmatic + agentic), A2A sessions plus Graph View activity.
...
**Scope:** Full fresh-install QA of Tsushin v0.6.0 on Ubuntu 24.04 VM (`10.211.55.5`, aarch64). Interactive installer executed via `expect` (no `--defaults` shortcut). All 31 TCs plus v0.6.0 feature coverage executed via both API (curl) and browser automation (Playwright). All integrations configured with real API keys: Gemini (default), OpenAI, Anthropic, Ollama (reconfigured to `0.0.0.0`), Brave Search, Tavily. Install path: `/home/parallels/code2/tsushin-v060-20260411-084511/`.

**Total findings: 6**
- **New:** `BUG-514`, `BUG-515`, `BUG-516`, `BUG-517`, `BUG-518`, `BUG-525`
- **Validated working paths:** health/readiness endpoints, tenant/global admin auth (API), all 15 settings sub-pages, Playground (chat, web search, image generation, KB, A2A delegation), API v1 OAuth2 + X-API-Key, Flows (create/list), Projects (create/conversation), Sandboxed tools (/tool dig, /status slash commands), Sentinel config, Qdrant auto-provisioning, A2A session graph data (1 completed session, 100% success rate), TC-23 memory isolation (isolated blocks cross-user, shared pool functional), TC-26 MCP server registration + tool discovery, TC-31 Graph View + A2A Comms tab (12 nodes / 20 connections, live green indicator).
...
**Scope:** Closeout of the remaining 8 open bugs from the 2026-04-10 Ubuntu VM fresh-install QA: `BUG-506`, `BUG-507`, `BUG-508`, `BUG-509`, `BUG-510`, `BUG-511`, `BUG-512`, `BUG-513`. All work on `develop`.

**Clusters:**
- **Cluster A — API v1 schema:** `BUG-506` (add `vertex_ai` to public v1 `model_provider` patterns; regen `docs/openapi.json`)
- **Cluster B — Frontend routing/UX:** `BUG-507` (Ollama health fallback), `BUG-508` (`/agents/[id]` NaN guard), `BUG-513` (Vertex Test disabled while dirty)
- **Cluster C — Custom skill runtime:** `BUG-509` (route instruction skills through LLM adapter at runtime + runtime execution history)
- **Cluster D — Slash commands:** `BUG-510` (`/shell` output now flows into `/inject` via pending-stub + lazy resolver)
- **Cluster E — Project chat:** `BUG-511` (shared `extract_response_text` helper propagates tool-only fallbacks into project conversation)
- **Cluster F — MCP bootstrap:** `BUG-512` (auto-start tenant toolbox container on stdio `/test`)

**Validation:** Safe rebuild (`docker-compose up -d --build --no-cache backend frontend` via two-step build), per-bug source-level probes (import + adapter behavior + buffer state transitions), OpenAPI schema regeneration, and Playwright verification of the 3 frontend fixes using real per-character keystrokes. Full platform regression via `/fire_regression` follows.

## Bug Sprint — 12 bugs resolved (2026-04-10)

**Scope:** Implemented the multi-cluster closeout for `BUG-388`, `BUG-419`, `BUG-465`, `BUG-466`, `BUG-467`, `BUG-499`, `BUG-500`, `BUG-501`, `BUG-502`, `BUG-503`, `BUG-504`, and `BUG-505` on `develop`.

**Validation:** Rebuilt `backend` and `frontend` without cache, verified backend health/readiness, ran authenticated API smoke checks, executed the focused integrated regression slice (`16 passed`), and used Playwright to validate Playground document rejection, threaded Playground round-trip/history/memory reads, and flow-wizard step selection stability.

## macOS v0.6.0 Targeted Regression (2026-04-10)

**Scope:** Full v0.6.0 feature regression on the production macOS dev stack (`develop` branch, `https://localhost`). All 22 TCs from the v0.6.0 test matrix executed via API (`X-API-Key` + OAuth2) and browser automation (Playwright). Provider configuration: Anthropic (`claude-sonnet-4-6`), OpenAI (`gpt-4o-mini`), Gemini (pre-configured), Vertex AI (pre-configured). Browser tests deferred pending Chrome CA cert restart (Caddy `tls internal` CA installed into macOS system keychain but Chrome required restart).

**API v1 E2E suite:** 24/29 pass — 5 failures in `TestAgentDescription` due to tenant `max_agents=5` but 16 agents exist (pre-existing BUG-447/related).

**Total findings: 2**
- **New:** `BUG-504`, `BUG-505`
- **Revalidated Open:** `BUG-499` (vector stores), `BUG-422` (flow message step), `BUG-495` (AgentNode flow)

### BUG-504: Custom skill invocation via API chat now returns the executed tool/skill output
- **Status:** Resolved
- **Resolved:** 2026-04-10
- **Reported:** 2026-04-10
- **Severity:** High
- **Category:** Custom Skills / API v1 / Agent Chat
- **Files:** `backend/agent/agent_service.py`, `backend/api/v1/routes_chat.py`
- **Description:** Tool-mode custom-skill executions could complete successfully while `/api/v1/agents/{id}/chat` returned `{"message":null,"response":null}`. The chat response pipeline now prefers non-empty tool/custom-skill output whenever the assistant text body is empty, preserving the existing response shape for both sync and async chat.
- **Fix:** `AgentService` now keeps successful tool/custom-skill results instead of collapsing them into an empty assistant message, and API v1 chat responses stay aligned with that fallback.
- **Verification:** Focused regressions `test_agent_service_prefers_tool_result_when_response_empty` plus the integrated 2026-04-10 regression slice passed.

### BUG-505: Sentinel audit logging now writes to `sentinel_analysis_log` with correct threat-only defaults
- **Status:** Resolved
- **Resolved:** 2026-04-10
- **Reported:** 2026-04-10
- **Severity:** Medium
- **Category:** Sentinel / Audit Logging
- **Files:** `backend/services/sentinel_service.py`
- **Description:** The tracker originally referenced `sentinel_audit_log`, but the live audit model/table is `sentinel_analysis_log`. Threat analyses must always be written there, while benign analyses should only be recorded when `log_all_analyses=true`.
- **Fix:** Sentinel now writes threat/fail-closed analyses to `sentinel_analysis_log` by default and only writes safe analyses when `log_all_analyses=true`.
- **Verification:** `TestAuditLogSemanticsSync` regressions passed in both the focused and integrated 2026-04-10 suites.

## Ubuntu VM Fresh-Install QA (2026-04-10)

**Scope:** Real interactive installer audit on Ubuntu VM `10.211.55.5` from fresh clone `/home/parallels/code/tsushin-v060-audit-20260410-164252`, with `/setup` completed via Playwright and final evidence stored under `.private/qa/ubuntu-vm-20260410/reports/` plus `output/playwright/`. Coverage included the installer transcript, auth/health/readiness, provider matrix (Gemini/OpenAI/Anthropic/Vertex AI/Brave/Tavily/Ollama), setup-time vector-store auto-provisioning, API-side memory isolation, Playground chat/memory, A2A + Graph View, MCP servers, custom skills, Shell Command Center, slash commands, API v1 generated-client validation, projects, and flows.

**Total findings:** 8
- **New:** `BUG-506`, `BUG-507`, `BUG-508`, `BUG-509`, `BUG-510`, `BUG-511`, `BUG-512`, `BUG-513`
- **Still fixed / not reproduced:** `BUG-388`, `BUG-419`, `BUG-499`, `BUG-500`
- **Revalidated working paths:** setup-time Qdrant auto-provisioning, API-side `isolated` / `shared` / `channel_isolated` memory behavior, real A2A session creation plus Graph View edge rendering, shell-beacon command execution, and generated SDK calls from `/api/v1/openapi.json`
- **Not tracked as product bugs:** tenant-admin `403` on `/system/tenants` remains expected RBAC behavior; the live public API thread retrieval path is still `GET /api/v1/agents/{agent_id}/threads/{thread_id}/messages`

### BUG-506: Public API v1 schema omits `vertex_ai` from `model_provider` even though the product supports it
- **Status:** Resolved
- **Resolved:** 2026-04-10
- **Reported:** 2026-04-10
- **Severity:** Medium
- **Category:** API v1 / OpenAPI / Provider Schema
- **Files:** `backend/api/v1/routes_agents.py`
- **Description:** The tenant product and internal agent APIs accept `model_provider="vertex_ai"`, but the public API v1 schema still constrained `model_provider` to `openai|anthropic|gemini|ollama|openrouter|groq|grok|deepseek`. Generated clients from `/api/v1/openapi.json` typed Vertex AI as invalid even though the release surface otherwise supports it.
- **Fix:** Added `vertex_ai` to the pattern in both `AgentCreateRequest.model_provider` (line 100) and `AgentUpdateRequest.model_provider` (line 163). Regenerated `docs/openapi.json` from the live backend so generated SDKs match.
- **Verification:** `curl -X POST .../api/v1/agents` with `model_provider=vertex_ai` no longer returns 422 on schema validation (hits agent-limit business logic instead). An invalid provider still returns 422. `/api/v1/openapi.json` now shows both request models carrying the `vertex_ai` branch.

### BUG-507: Agents UI still probes a nonexistent frontend-local Ollama health route
- **Status:** Resolved
- **Resolved:** 2026-04-10
- **Reported:** 2026-04-10
- **Severity:** Medium
- **Category:** Frontend / Ollama / Studio
- **Files:** `frontend/lib/client.ts`
- **Description:** Fresh-install agent-management surfaces called `/api/ollama/health` via `api.getOllamaHealth()` and threw on 4xx, surfacing noisy `404` console errors whenever Ollama wasn't configured (or when the client resolved the wrong origin). The backend Ollama endpoint is reachable on a healthy install; the UI should never turn an optional health check into a user-visible error.
- **Fix:** Wrapped `getOllamaHealth()` in a try/catch that returns `{available: false, models: []}` on any fetch or non-OK response, so the call never throws. All four callers (`frontend/app/agents/page.tsx:317`, `frontend/app/hub/page.tsx:833`, `frontend/components/AgentConfigurationManager.tsx:235`, `frontend/components/playground/ConfigPanel.tsx:208`) already handle the fallback shape.
- **Verification:** Playwright sweep of `/agents` with DevTools console open shows zero red console errors related to `/api/ollama/health` post-rebuild.

### BUG-508: `/agents/communication` is routed through the dynamic agent detail page and crashes with `/api/agents/NaN`
- **Status:** Resolved
- **Resolved:** 2026-04-10
- **Reported:** 2026-04-10
- **Severity:** High
- **Category:** A2A Communication / Frontend Routing
- **Files:** `frontend/app/agents/[id]/page.tsx`
- **Description:** There is no dedicated `/agents/communication` page, so Next.js resolved that URL through the dynamic agent-detail route. `parseInt("communication")` yielded `NaN`, the page requested `/api/agents/NaN`, raised the alert `Failed to load agent details`, and redirected back to `/agents`.
- **Fix:** Guarded the dynamic `[id]` route with a `Number.isFinite` check. When the parsed id is invalid the effect immediately calls `router.replace('/agents')`, the fetch is skipped, and the component renders `null` while the redirect is scheduled — no more 422, no more alert, no doomed network request.
- **Verification:** Playwright navigation to `https://localhost/agents/communication` redirects cleanly to `/agents` with zero console errors and no `/api/agents/NaN` in the network tab; `/agents/1` still loads normally.

### BUG-509: Assigned instruction custom skills leak their instructions in Playground chat instead of executing
- **Status:** Resolved
- **Resolved:** 2026-04-10
- **Reported:** 2026-04-10
- **Severity:** High
- **Category:** Custom Skills / Playground / Runtime Invocation
- **Files:** `backend/agent/skills/custom_skill_adapter.py`, `backend/agent/skills/skill_manager.py`
- **Description:** An instruction custom skill tested successfully through `POST /api/custom-skills/{id}/test` (via the `execute_instruction_with_llm` adapter), but invoking the same assigned skill from Playground chat returned the raw `instructions_md` text. Two disconnected code paths were at fault: (1) `get_custom_skill_instructions` injected raw instruction markdown into the agent's system prompt regardless of execution mode, so the LLM parroted it; and (2) when the LLM did choose to call the skill as a tool, `CustomSkillAdapter.execute_tool` routed to `_execute_instruction` which also returned the raw template.
- **Fix:**
  1. `CustomSkillAdapter.execute_tool` now routes instruction-variant skills through `execute_instruction_with_llm(arguments, config)`, matching the `/test` endpoint behavior at runtime.
  2. `SkillManager.get_custom_skill_instructions` now only injects raw instruction text for skills with `execution_mode == 'passive'`. Tool/hybrid instruction skills are exposed as callable tools via `get_custom_skill_tool_definitions` and execute through the LLM adapter when invoked, never leaking their template.
  3. `SkillManager.execute_tool_call` now writes a `CustomSkillExecution` history row for every runtime custom-skill invocation (mirroring the `/test` endpoint pattern), so assigned skills appear in execution history alongside direct test calls.
- **Verification:** Direct adapter probe confirms `execute_tool({}, None, {})` on an instruction skill returns `"No database session available for LLM execution"` instead of the raw `instructions_md` — proving the new LLM path is reached. Full end-to-end Playground verification will run in the 2026-04-10 regression sweep.

### BUG-510: Completed `/shell` executions are still invisible to `/inject`
- **Status:** Resolved
- **Resolved:** 2026-04-10
- **Reported:** 2026-04-10
- **Severity:** High
- **Category:** Slash Commands / Shell Command Center / Context Injection
- **Files:** `backend/services/slash_command_service.py`, `backend/agent/memory/tool_output_buffer.py`
- **Description:** `/inject list` only reads `ToolOutputBuffer`, but the `/shell` handler returned its result with `action="shell_queued"`/`"shell_executed"` — neither of which the buffer recognized — and never wrote shell output to the buffer. Async executions were doubly invisible: by the time the beacon completed and wrote to the `ShellCommand` table, nothing pulled that result back into the in-memory buffer.
- **Fix:**
  1. Extended `ToolExecution` with optional `pending`/`source`/`source_ref` fields and added `list_pending_executions` + `update_execution_output` helpers on `ToolOutputBuffer`.
  2. `_handle_shell` now buffers every `/shell` call: for `wait_for_result=True` it buffers the final stdout/stderr immediately; for fire-and-forget it writes a pending stub tagged `source='shell_command'` with the beacon command id in `source_ref`.
  3. Added `_resolve_pending_shell_executions` to `SlashCommandService`, called at the top of `_handle_inject` (capped at 10 entries per call). It walks pending shell stubs, queries `ShellCommandService.get_command_result`, and rewrites the buffered output once the beacon marks the command `completed`/`failed`/`timeout`.
- **Verification:** Inline buffer probe confirms `add_tool_output`/`list_pending_executions`/`update_execution_output` are wired correctly and pending → resolved transitions work. Full `/shell` + `/inject` round-trip runs in the regression sweep.

### BUG-511: Project conversation chat returns success without an assistant reply even when project fact and knowledge are present
- **Status:** Resolved
- **Resolved:** 2026-04-10
- **Reported:** 2026-04-10
- **Severity:** High
- **Category:** Projects / Knowledge Base / Chat
- **Files:** `backend/services/project_service.py`, `backend/api/v1/routes_chat.py`, `backend/agent/response_helpers.py` (new)
- **Description:** `ProjectService.send_message` appended the assistant reply only if `response.get("message")` was truthy and returned that same field unchanged. When `PlaygroundService` produced a tool-only result (empty `message`/`answer`, real content in `tool_result`) — the exact pattern BUG-504 already fixed in API v1 chat — project conversations silently dropped the reply. The caller got `{"status":"success","message":null}` even though the LLM had answered.
- **Fix:** Promoted the BUG-504 helper `_extract_response_text` into a shared module `backend/agent/response_helpers.py`. `backend/api/v1/routes_chat.py` now imports it, and `ProjectService.send_message` uses the same helper to compute `assistant_text` and returns it both in the message append and the final response envelope. Existing BUG-504 API v1 regressions stay green.
- **Verification:** Inline import test confirms the shared helper returns the tool-result fallback on empty primary fields and the plain message when populated. End-to-end project-chat test runs in the regression sweep.

### BUG-512: Fresh-install stdio MCP testing still depends on a toolbox container that the UI/API never boots for the user
- **Status:** Resolved
- **Resolved:** 2026-04-10
- **Reported:** 2026-04-10
- **Severity:** Medium
- **Category:** MCP / Toolbox / Fresh Install
- **Files:** `backend/hub/mcp/connection_manager.py`
- **Description:** `POST /api/mcp-servers/{id}/test` → `MCPConnectionManager.get_or_connect` → `StdioTransport.connect()` → `ToolboxContainerService.execute_command()` raised `RuntimeError("Container not found for tenant ... Please start it first.")` on fresh installs that had never booted the tenant's toolbox container. There was no first-class UI action to bootstrap it, so stdio MCP validation dead-ended.
- **Fix:** `MCPConnectionManager.get_or_connect` now calls `ToolboxContainerService.ensure_container_running(tenant_id, db)` for any `transport_type == 'stdio'` config before constructing the transport. This creates (or starts) the tenant's toolbox container on demand so first-time `/test` calls — and first-time runtime stdio invocations — succeed instead of erroring with a raw container message. Failures bubble up as a clear `RuntimeError("Failed to start toolbox container for tenant ...")`.
- **Verification:** Import probe confirms the bootstrap is wired in `get_or_connect` and `ensure_container_running` is available on `ToolboxContainerService`. Full stdio-MCP `/test` verification with a torn-down toolbox runs in the regression sweep.

### BUG-513: Hub Vertex AI modal tests only the saved tenant config, not the values currently typed into the modal
- **Status:** Resolved
- **Resolved:** 2026-04-10
- **Reported:** 2026-04-10
- **Severity:** Low
- **Category:** Hub / Vertex AI / UX
- **Files:** `frontend/app/hub/page.tsx`
- **Description:** The Vertex AI modal `Test Connection` handler posted an empty body to `/api/integrations/vertex_ai/test`, which reads saved API keys from the DB. Users could fully populate the unsaved form, press Test, and get the previously saved config validated instead of their new fields — misleading during first-time setup.
- **Fix (UI-only, no new backend surface area):** Added a `vertexDirty` state flag that flips to `true` on any form edit (Project ID / Region / SA email / Private Key / JSON paste). `Save Configuration` resets the flag on success. The `Test Connection` button is now disabled while `vertexDirty` is true, with helper text `"Save configuration before testing — the Test Connection button validates the currently saved credentials, not the fields above."` rendered above the action row. The alternative — extending the backend test endpoint to accept draft credentials — was rejected because it would add a new path where raw secrets flow through a test surface.
- **Verification:** Playwright probe using real per-character keystrokes (`pressSequentially`, which fires React onChange) confirms: button is disabled on open (empty private key), stays disabled after typing any field (dirty=true), helper text appears on first keystroke, DOM snapshot shows `<button disabled="" class="btn-secondary flex-1">Test Connection</button>`.

## macOS v0.6.0 Full Feature Regression (2026-04-09)

**Scope:** Comprehensive fresh-install feature regression on macOS (develop branch, v0.6.0). Disposable install at `/Users/vinicios/tsushin-install-test/tsushin` with `TSN_STACK_NAME=tsushin-fresh python3 install.py --defaults --http`. All 23 test cases (TC-01 to TC-23) executed via API and browser automation. WhatsApp QR / messaging tests skipped per plan. Coverage: infrastructure health, auth, agents, memory, knowledge base, Sentinel, playground (chat/image/memory), A2A, MCP, custom skills, shell integrations, sandboxed tools (dig, nmap), slash commands (/status, /inject, /clear), API v1 OAuth2, flows (7 templates), channel health, webhooks, browser automation skill, settings pages, system admin.

**Total findings: 4** (Medium: 2, Low: 2)

### BUG-495: Flow executor rejects "AgentNode" step type accepted by API
- **Status:** Resolved
- **Reported:** 2026-04-09
- **Resolved:** 2026-04-09
- **Severity:** Medium
- **Category:** Flows / Execution Engine
- **Files:** `backend/flows/flow_engine.py`, `backend/api/routes_flows.py`
- **Description:** The flow node creation endpoint (`POST /api/flows/{id}/nodes`) accepts `type="AgentNode"` and returns HTTP 201. However, the flow execution engine has no registered handler for this type and raises `"No handler for step type: AgentNode"` at runtime, failing the flow run immediately. The disconnect between accepted types and supported types is silent — there is no validation at creation time. Supported handler types are: notification, message, tool, conversation, slash_command, skill, custom_skill, summarization, gate, browser_automation (and their PascalCase variants).
- **Steps to Reproduce:** 1) `POST /api/flows` to create a flow. 2) `POST /api/flows/{id}/nodes` with `{"type":"AgentNode","position":2,"config_json":{"agent_id":1,"message":"test"}}` → 201. 3) `POST /api/flows/{id}/nodes` with Trigger at position 1. 4) `POST /api/flows/{id}/run` → 202. 5) Poll run — status="failed", error="No handler for step type: AgentNode".
- **Remediation:** Added `"AgentNode": ConversationStepHandler(...)` to the handlers dict in `flow_engine.py`. `AgentNode` is now an alias for `ConversationStepHandler` (same config schema: `agent_id`, `message`). Verified: flow run no longer fails with "No handler" error.

### BUG-496: Summarization flow step silently ignores inline text, requires thread_id
- **Status:** Resolved
- **Reported:** 2026-04-09
- **Resolved:** 2026-04-09
- **Severity:** Low
- **Category:** Flows / Execution Engine
- **Files:** `backend/flows/flow_engine.py` (SummarizationStepHandler)
- **Description:** The `Summarization` flow step type only supports `thread_id` (to summarize an existing conversation thread) or `source_step` references. When a user supplies a `text` or `content` key in `config_json` (e.g., to summarize arbitrary text inline), those keys are silently ignored and the step fails with "No thread_id or source text found". This makes it impossible to use flows for summarizing arbitrary inline text without first running a conversation step.
- **Steps to Reproduce:** 1) Create flow with Trigger + Summarization nodes. 2) Set Summarization `config_json = {"agent_id":1,"text":"Some text to summarize"}`. 3) Run flow → step fails: "No thread_id or source text found".
- **Remediation:** Added `config.get("text") or config.get("content")` check immediately after `source_text = None` in `SummarizationStepHandler.execute()`. Inline text in `config_json` is now picked up before falling back to `source_step`/`previous_step`. Also updated the fallback error message to mention `text`/`content` as valid config keys. Verified: step now proceeds to AI generation instead of failing with "No thread_id" error.

### BUG-497: Semantic memory search crashes with NumPy array ambiguity error
- **Status:** Resolved
- **Reported:** 2026-04-09
- **Resolved:** 2026-04-09
- **Severity:** Medium
- **Category:** Memory / Vector Store
- **Files:** `backend/agent/memory/vector_store.py` (search_similar_with_embeddings)
- **Description:** Backend logs show recurring errors during playground conversations: `"Error in search_similar_with_embeddings: The truth value of an array with more than one element is ambiguous. Use a.any() or a.all()"`. This is a NumPy boolean array comparison bug — likely a bare `if array:` check on a multi-element ndarray result. The error is non-fatal (search falls back gracefully), but semantic memory recall is silently degraded whenever the bug is triggered.
- **Steps to Reproduce:** 1) Chat in playground with an agent that has memory enabled. 2) Check backend logs: `docker logs tsushin-backend --tail=30 | grep "array"`. 3) Observe "The truth value of an array with more than one element is ambiguous" error.
- **Remediation:** Fixed bare ndarray boolean check in `vector_store.py:300` — replaced `if results['embeddings'][0]:` with explicit `is not None and len(...) > 0` check. Also added `float()` wrapping around `np.linalg.norm()` calls in `embedding_service.py` and `temporal_decay.py` to prevent numpy scalar comparisons. Verified: no "ambiguous" errors in backend logs after fix.

### BUG-498: Custom script skill rejects function names as script_entrypoint (must be filename)
- **Status:** Resolved
- **Reported:** 2026-04-09
- **Resolved:** 2026-04-09
- **Severity:** Low
- **Category:** Custom Skills / API
- **Files:** `backend/api/routes_custom_skills.py`
- **Description:** When creating a script-based custom skill via `POST /api/custom-skills`, the `script_entrypoint` field validation requires a filename ending in `.py`, `.sh`, or `.js` (e.g., `run.py`). However, the field name "script_entrypoint" conventionally implies a function or method name (e.g., `run`). The error message is: "script_entrypoint must be a simple filename ending in .py, .sh, or .js". This naming mismatch confuses API users who supply a function name instead.
- **Steps to Reproduce:** 1) `POST /api/custom-skills` with `{"skill_type":"script","script_language":"python","script_content":"...","script_entrypoint":"run"}` → 400 "script_entrypoint must be a simple filename".
- **Remediation:** Updated validation in `routes_custom_skills.py` (both CREATE and UPDATE blocks) to accept bare function names (e.g., `"run"`) in addition to full filenames (e.g., `"run.py"`). When a bare name is supplied without an extension, it is auto-extended based on `script_language` (python→`.py`, bash→`.sh`, nodejs→`.js`). Security constraint (no path traversal) retained. Verified: `"run"` accepted and stored as `"run.py"` on creation.

## macOS Fresh Install Memory / Shell / Runtime Follow-up (2026-04-09)

**Scope:** Continued the interactive disposable-clone audit on macOS at `/Users/vinicios/code/tsushin/.private/installations/fresh-install-v060-20260409-205335/tsushin` after setup, provider configuration, API v1, A2A, MCP, custom skills, and shell validation were already passing. Coverage in this follow-up focused on auto-provisioned vector stores, thread-scoped Playground memory/history, `/tool` + `/inject` interoperability, runtime naming consistency for Kokoro, and re-validating the open flow regressions before the WhatsApp phase.

**New bugs found: 4** (High: 2, Medium: 1, Low: 1)
**Revalidated:** BUG-466
**Not reproduced:** BUG-467 (wizard stayed open and rendered the Message step editor correctly on this build)

### BUG-499: Fresh installs no longer auto-provision a default vector store instance
- **Status:** Resolved
- **Resolved:** 2026-04-10
- **Severity:** High
- **Category:** Memory / Vector Stores / Fresh Install
- **Files:** `backend/auth_routes.py`, `backend/api/routes_vector_stores.py`, `backend/services/vector_store_instance_service.py`
- **Description:** Fresh setup now provisions `Qdrant (Default)` through the shared vector-store helper instead of leaving long-term memory unconfigured. If provisioning fails, setup completes with a visible warning and manual recovery path rather than aborting installation.
- **Fix:** Added `VectorStoreInstanceService.create_default_setup_instance()` and invoked it from the setup wizard with fail-open warning semantics.
- **Verification:** `test_setup_wizard_surfaces_default_vector_store_warning`, `test_create_default_instance_warns_when_provisioning_fails`, and `test_create_default_instance_returns_existing_default` passed in the integrated 2026-04-10 regression slice.

### BUG-500: Playground sync threads drop the first user message, breaking same-thread recall and thread-scoped history
- **Status:** Resolved
- **Resolved:** 2026-04-10
- **Severity:** High
- **Category:** Playground / Memory / Threading
- **Files:** `backend/api/routes_playground.py`, `backend/services/playground_service.py`, `backend/services/playground_thread_service.py`
- **Description:** Playground sync chat, queue/streaming, thread detail, history, and memory reads now all use the same canonical thread identity. The initial user turn is persisted before the assistant response, and `thread_id` reads resolve the correct thread-scoped sender key instead of drifting to an unrelated fallback buffer.
- **Fix:** Centralized `resolve_playground_identity()` in the thread service and reused it across writes, reads, websocket thread refresh, and queue processing.
- **Verification:** `backend/tests/test_playground_memory_regressions.py` passed, Playwright completed a live threaded round-trip on thread `7`, and authenticated API smoke returned both user/assistant turns plus thread-scoped working memory via `/api/playground/history/1?thread_id=7` and `/api/playground/memory/1?thread_id=7`.

### BUG-501: `/tool` executions are not exposed to `/inject`
- **Status:** Resolved
- **Resolved:** 2026-04-10
- **Severity:** Medium
- **Category:** Slash Commands / Sandboxed Tools
- **Files:** `backend/services/slash_command_service.py`, `backend/agent/memory/tool_output_buffer.py`
- **Description:** Successful `/tool` executions are now buffered once in the shared slash-command path, so `/inject` can retrieve them regardless of whether the command ran via the direct route, Playground sync, or queue/streaming.
- **Fix:** Moved tool-output buffering into `SlashCommandService` and made the tool buffer idempotent so route-level duplication could be removed.
- **Verification:** `backend/tests/test_v060_tooling_regressions.py` passed, including the `/tool` → `/inject` regression slice.

### BUG-502: Kokoro still uses the global `kokoro-tts` hostname instead of the stack-scoped service name
- **Status:** Resolved
- **Resolved:** 2026-04-10
- **Severity:** Low
- **Category:** Runtime Isolation / Naming Consistency
- **Files:** `docker-compose.yml`, `install.py`, `backend/hub/providers/kokoro_tts_provider.py`
- **Description:** Kokoro now resolves through a stack-scoped hostname derived from `TSN_STACK_NAME`, preventing cross-stack collisions on the shared Docker network.
- **Fix:** Updated compose defaults, installer env backfill, and the provider fallback URL to use `${TSN_STACK_NAME:-tsushin}-kokoro-tts`.
- **Verification:** The integrated regression slice covered the installer/provider cases, `docker exec tsushin-backend` showed `KOKORO_SERVICE_URL=http://tsushin-kokoro-tts:8880`, and `/api/tts-providers/kokoro/status` reported that same stack-scoped URL at runtime.
...
- **Fix:** Playground now resolves `channel_isolated` memory through a canonical per-channel sender key while keeping thread-specific recipients and history filtering for individual threads.
- **Verification:** `backend/tests/test_playground_memory_regressions.py` covers the per-channel recall contract and passed in the integrated 2026-04-10 suite.

### BUG-503: A2A delegation skill returns an agent list but does not create a communication session
- **Status:** Resolved
- **Resolved:** 2026-04-10
- **Reported:** 2026-04-10
- **Severity:** Medium
- **Category:** A2A Communication / Playground / Watcher
- **Files:** `backend/agent/skills/agent_communication_skill.py`, `backend/agent/agent_service.py`
- **Description:** Delegate-intent `list_agents` calls now progress into a real A2A session/request/response cycle instead of stopping at agent discovery.
- **Fix:** The A2A skill promotes delegate intent into session creation when permissions already allow the target agent, and the agent service preserves the provider-instance context for that handoff.
- **Verification:** `test_invoke_target_agent_preserves_provider_instance_id` and `test_list_agents_delegate_intent_creates_real_session` passed in the focused and integrated 2026-04-10 suites.
...
- **Fix:** Shared-memory Playground traffic now forces the canonical `shared` sender key even when route or queue callers provide a sender override, while thread-scoped history remains filtered by thread metadata.
- **Verification:** `backend/tests/test_playground_memory_regressions.py` passed and the live 2026-04-10 Playground smoke confirmed thread-scoped history/memory reads stayed aligned with the underlying memory contract.
...
## Ubuntu VM Fresh Install Full QA (2026-04-09)

### BUG-465: Playground "Attach documents" accepts PNG/image files but LLM cannot process them
- **Status:** Resolved
- **Resolved:** 2026-04-10
- **Severity:** Medium
- **Category:** Playground / Document Uploads
- **Files:** `frontend/components/playground/DocumentPanel.tsx`, `frontend/components/playground/ExpertMode.tsx`, `backend/services/playground_document_service.py`
- **Description:** The Playground document flow now rejects image uploads instead of silently accepting files the backend cannot process. Supported-type copy in the UI matches the backend validator.
- **Fix:** Tightened the frontend `accept`/validation rules and kept the backend extension whitelist authoritative for document-only uploads.
- **Verification:** Playwright 2026-04-10 uploaded a `.png` and the UI returned `Unsupported file type: .png. Supported: .pdf, .txt, .csv, .json, .xlsx, .xls, .docx, .doc, .md, .markdown, .rtf`.

### BUG-466: Flow message step execution fails with `'NoneType' object is not iterable`
- **Status:** Resolved
- **Resolved:** 2026-04-10
- **Severity:** High
- **Category:** Flows / Backend
- **Files:** `backend/schemas.py`, `backend/api/routes_flows.py`, `backend/flows/flow_engine.py`
- **Description:** Message-step configs now normalize `recipients` to an empty list when legacy payloads or the UI send `null`, then safely merge any singular `recipient` value before runtime execution.
- **Fix:** Added normalization at schema, route serialization, and flow-execution boundaries so `recipient`-only message steps no longer persist or execute with `recipients: null`.
- **Verification:** `backend/tests/test_routes_flows_alias.py` passed in the integrated 2026-04-10 regression slice.

### BUG-467: Flow creation wizard Step 2 closes prematurely when step type is clicked
- **Status:** Resolved
- **Resolved:** 2026-04-10
- **Severity:** Medium
- **Category:** UI / Flows
- **Files:** `frontend/app/flows/page.tsx`
- **Description:** Step-type selection in the flow-create wizard now stays inside the wizard and does not submit or dismiss the panel prematurely, even with the onboarding/user-guide overlays active.
- **Fix:** Hardened the wizard/modal event handling so step-type clicks only add the selected step editor and `Create Flow` remains an explicit final action.
- **Verification:** Playwright 2026-04-10 completed `New Flow` → `Continue` → `Message` step selection on `/flows` while the User Guide overlay was present, and the wizard remained open with the inline Message-step editor rendered.
